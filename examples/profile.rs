...
fn main() {
    ...
    println!(
        "Char iteration only: {:.1}ms/iter ({} chars)",
        t1.as_secs_f64() * 1000.0 / 100.0,
        n
    );
    ...
    println!(
        "Full convert:        {:.1}ms/iter",
        t2.as_secs_f64() * 1000.0 / 100.0
    );
    ...
    println!(
        "convert_to (reuse):  {:.1}ms/iter",
        t3.as_secs_f64() * 1000.0 / 100.0
    );
